...
use App\Exports\Gestiones\GestionesPropia12Export;
use Carbon\Carbon;
use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use Maatwebsite\Excel\Facades\Excel;

class GestionesPropia12ReportController extends Controller
{
    public function index(Request $request)
    {
        ...
        $documento    = trim((string) $request->query('documento', ''));    // documento (DNI)
        ...
        return view('reportes.gestiones.propia12', compact(
            'desde', 'hasta', 'telefono', 'documento', 'tipificacion', 'registros'
        ));
    }
    ...
    public function xlsx(Request $request)
    {
        ...
        $filename = "gestiones_propia12_{$desde}_{$hasta}.xlsx";

        return Excel::download(
            new GestionesPropia12Export($desde, $hasta, $telefono, $documento, $tipificacion),
            $filename
        );
    }
}
