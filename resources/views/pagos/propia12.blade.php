...
<div class="space-y-5">
    ...
    <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
        ...
        <a href="{{ route('pagos.propia12.template') }}"
    ...
    {{-- GRID: izquierda (upload+buscar) / derecha (manual) --}}
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-12">
        ...
        <div class="lg:col-span-5 space-y-4">
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                <div class="flex items-start justify-between gap-3">
                    <div>
                        ...
                        <div class="mt-0.5 text-xs text-slate-500">
                            Importa un CSV (delimitado por comas) para Propia 1 y 2.
                        </div>
                    </div>
                ...
                <form method="POST" action="{{ route('pagos.propia12.upload') }}" enctype="multipart/form-data" class="mt-4 space-y-3">
                    @csrf

                    <div class="rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-4">
                        <label class="block text-xs font-semibold text-slate-700">Archivo CSV</label>
                        <input
                            type="file"
                            name="archivo"
                            required
            ...
            <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                <div class="flex items-start justify-between gap-3">
                    <div>
                        <div class="text-sm font-semibold text-slate-900">Buscar pagos / cliente</div>                    </div>
                    @if($dni)
                        <span class="inline-flex items-center rounded-lg border border-slate-200 bg-slate-50 px-2 py-1 text-xs font-semibold text-slate-700">
                            Filtro
                        </span>
                ...
                <form method="GET" action="{{ route('pagos.propia12.index') }}" class="mt-4 space-y-3">
                    <div>
                    ...
                    <div class="flex gap-2">
                        ...
                        @if($dni)
                            <a
                                href="{{ route('pagos.propia12.index') }}"
        ...
        <div class="lg:col-span-7">
            <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                <div>
                    ...
                    <div class="mt-0.5 text-xs text-slate-500">
                        Úsalo para registrar un pago puntual o corregir información.
                    </div>
                </div>

                <form method="POST" action="{{ route('pagos.propia12.store') }}" class="mt-4">
                    @csrf

                    <div class="grid grid-cols-1 gap-3 sm:grid-cols-12">
                        <div class="sm:col-span-3">
                            <label class="text-xs font-semibold text-slate-700">DNI</label>
                            <input type="text" name="dni" required inputmode="numeric"
                        ...
                        <div class="sm:col-span-3">
                            ...
                            <select name="moneda"
                                    class="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm outline-none focus:ring-4 focus:ring-slate-100">
                        ...
                        <div class="sm:col-span-5">
                            ...
                            <input type="text" name="gestor" placeholder="Opcional"
                                   class="mt-1 w-full rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:ring-4 focus:ring-slate-100">
                        </div>
    ...
    <div class="rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div class="flex items-center justify-between gap-3 border-b border-slate-200 px-4 py-3">
            <div>
                ...
                <div class="text-xs text-slate-500">Edita o elimina registros rápidamente.</div>
            </div>
        </div>
                                ...
                                <div x-data="{ open:false }" class="inline-flex items-center gap-2">
                                    ...
                                    <div x-show="open" x-transition.opacity style="display:none;"
                                        ...
                                        <div class="relative w-full max-w-lg rounded-2xl border border-slate-200 bg-white shadow-xl">
                                            ...
                                            <form method="POST" action="{{ route('pagos.propia12.update', $p->id) }}">
                                                @csrf
        ...
        <div class="flex flex-col gap-2 border-t border-slate-200 px-4 py-3 sm:flex-row sm:items-center sm:justify-between">
            ...
            {{-- usa paginación tailwind --}}
            <div class="text-sm">
                {{ $pagos->onEachSide(1)->links('vendor.pagination.tailwind') }}
            </div>
        </div>
    </div>
